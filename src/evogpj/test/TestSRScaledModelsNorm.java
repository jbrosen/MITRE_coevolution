...
public class TestSRScaledModelsNorm {
     ...
     */
    public TestSRScaledModelsNorm(String aPathToData, String aPathToTrainBounds, String aPathToTestBounds, String aPathToPop,boolean aRound) throws IOException, ClassNotFoundException {
        ...
        readBoundsTrain(pathToTrainBounds);
        readBoundsTest(pathToTestBounds);
    }
    ...
    public void evalPop() {
        ...
        for(Individual ind:models){
            ...
            for (int i = 0; i < data.getNumberOfFitnessCases(); i++) {
                ...
                double unscaledPrediction = unscale(val,minTargetTr,maxTargetTr);
                double unscaledTarget = unscale(targets[i],minTargetTest,maxTargetTest);
                sqDiff += Math.pow(unscaledTarget - unscaledPrediction, 2);
                absDiff += Math.abs(unscaledTarget - unscaledPrediction);
                d.clear();
            }
